...
- **A rolled color scheme.** Two hues, `--accent` and `--flair2`, plus a matching `--accent-soft`. Saturation rolls freely, and lightness is fitted per hue so the text accent clears 4.5:1 on the page, card, and tinted surfaces in both themes. the decorative secondary color clears 3:1 on the page background. It overrides the template's palette from a `<style id="flair">` block at the end of `<head>`, so the original values stay in the file untouched.
...
- **A real quote** in a colophon before `</main>`, drawn at random from a wide pool spanning science, literature, philosophy, art, sport and design.

the required model signature follows the quote. stamp it with `scripts/stamp-signature.py` after the report and flair are final. reapplying flair preserves that order.

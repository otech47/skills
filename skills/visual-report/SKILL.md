...
- Optional decorative chrome, a rolled accent hue, a corner badge, a small corner mascot, and a one-line colophon phrase, can run on top of a finished report. None of it is information: it never sits in the reading column, never touches the thesis, hero, or body copy, and cannot be wrong because it makes no claim about the report's content. It lives in `references/flair-chrome.md`, present means on, renamed to `.off` means skip it entirely. Read that file for the exact mechanism, the real-entropy script that rolls its numbers, and the one line you invent yourself. Knob: `report-flair on|off|status`.

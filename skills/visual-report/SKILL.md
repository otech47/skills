...
- Optional decorative chrome can run on top of a finished report: a rolled accent hue, a corner ornament (a ring, a pulsing gem, three twinkling sparks, no face), and an off-by-one quote in a colophon at the very bottom. None of it is information. It never sits in the reading column, never touches the thesis, hero, or body copy, and cannot be wrong because it makes no claim about the report's content. It lives in `references/flair-chrome.md`. Present means on, renamed to `.off` means skip it entirely. Read that file for the exact mechanism, the real-entropy script that rolls its numbers, and the one quote you invent yourself. Knob: `report-flair on|off|status`.
